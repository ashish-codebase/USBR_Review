...
def get_db(all_site_data:DB_Reader, site_name) -> pd.DataFrame:
    daqm_data = pd.DataFrame()
    start_time = process_time()
    if site_name == 'Baggs':
        daqm_data = all_site_data.baggs

    if site_name == 'Boulder':
        daqm_data = all_site_data.boulder

    if site_name == 'Cora':
        daqm_data = all_site_data.cora

    if site_name == 'Cortez':
        daqm_data = all_site_data.cortez   

    if site_name == 'Farson':
        daqm_data = all_site_data.farson        

    if site_name == 'FtBridger':
        daqm_data = all_site_data.ftBridger        

    if site_name == 'Gunnison':
        daqm_data = all_site_data.gunnison        

    if site_name == 'LaPlata':
        daqm_data = all_site_data.laPlata    

    if site_name == 'NAPI':
        daqm_data = all_site_data.nAPI        

    if site_name == 'Olathe':
        daqm_data = all_site_data.olathe        
...
def main(all_site_data):
    ...
    merged_df = get_db(all_site_data, str(selected_site))
    if merged_df.empty:
    ...
    limit_date = today_end - pd.Timedelta(days=days_limit)
    ...
    if data_max_date < limit_date:
        ...
        st.markdown(
            f"#### Data requested from: {datetime.date(limit_date)} to: {datetime.date(today_end)}"
        )
        st.stop()
    ...
    merged_df = merged_df[
        ...
        (merged_df.index >= date_range[0]) & (merged_df.index <= today_end)
    ...
    date_range_placeholder.markdown(
        f"- **Start time:** {datetime.date(merged_df.index[0])}; **End time:** {today_end:%Y-%m-%d  %H:%M %p}")
    ...
    st.text("Timing difference between GHG and SF3. If the system is working normally, the values should be 0 or 1. But when SF3 doesn't communicate with installed GPS reciever to get unix time, the values are -1. If no time stamp is found at all (i.e. corrupt file, which rarely happens).")
    st.divider()
