...
def get_db(table_name) -> pd.DataFrame:
...
def main():
    ...
    merged_df = get_db(str(selected_site))
    if merged_df.empty:
    ...
    st.divider()
    st.image(f"./Data/Offset/{selected_site}_offset.png", caption="Timing difference (s) between GHG and Sonic",use_container_width=True)
