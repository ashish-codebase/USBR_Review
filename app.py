...
def plot_co2signal(df:pd.DataFrame, date_range, selected_site_bold):
...
def main(all_site_data):
    ...
    # st.write("Selected data's rows count: ",merged_df.shape[0])
    ...
    custom_range = st.slider(
        ...
        max_value=lastIndex,
        step=144,
        value=(int(3*lastIndex/4),lastIndex)
    )

    st.divider()
    st.write("#### Selected date range:", merged_df.index[custom_range[0]], merged_df.index[custom_range[1]])
    ...
    date_range_placeholder.markdown(
        f"- #### **Start time:** {first_valid_index}; **End time:** {last_valid_index}")
    ...
    plot_co2signal(merged_df, date_range, selected_site_bold)
