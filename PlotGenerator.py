...
class PlotGenerator:
    ...
    def plot_t_et_vpd(self) -> None:
        ...
        ax1.set_title(self.selected_site_bold + ": Daily ET, Tmax & VPDmax.")
    ...
    def plot_rads_ratios(self):
        ...
        ax.plot(LW_Delta_median.index, LW_Delta_median, linewidth=0.5, color="blue", label=f'LW Delta: {LW_mean_value}', marker='o', markersize=2)
        ax.plot(SW_Delta_median.index, SW_Delta_median, linewidth=0.5, color="green", label=f'SW Delta: {SW_mean_value}', marker='o', markersize=2)
        ...
        ax.set_title(
            ...
            self.selected_site_bold + ": SWIN - SWOUT; LWOUT-LWIN (W/m2)" 
        )
    ...
    def sf3_json_stats(self):
        ...
        styled_df = df.style.applymap(color_col_a, subset=['USB free space'])\
                            .applymap(color_col_b, subset=['Offset'])\
                            .applymap(color_col_c, subset=['GPS detected'])\
                            .applymap(color_col_M, subset = ['M'])
