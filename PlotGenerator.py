...
class PlotGenerator:
    ...
    def plot_fetch(self):
        try:
            df = self.df.copy()
            df['ET'] = df['ET'].where(df['SWIN_1_1_1'] > 25, np.nan)
            df['ET'] = df['ET'].where(df['u*'] > 0.15, np.nan)
            df.dropna(subset=['wind_dir', 'x_90%', 'x_70%', 'x_50%', 'x_30%','ET'], inplace=True)
            ...
            distance_30 = df['x_30%']
            scaler = MinMaxScaler()
            df['ET_normalized'] = scaler.fit_transform (df[['ET']])
            ET_normalized = df['ET_normalized']
            # Convert degrees to radians for polar plot
            wind_direction_rad = np.deg2rad(wind_direction_deg)
            fig, ax = plt.subplots(subplot_kw={'projection': 'polar'}, figsize=(6,6))
            # Scatter plot
            ax.scatter(wind_direction_rad, distance_90, s=ET_normalized*600, alpha=0.045, label='90%',color='red')
            ax.scatter(wind_direction_rad, distance_70, s=ET_normalized*600, alpha=0.045, label='70%',color='green')
            ax.scatter(wind_direction_rad, distance_50, s=ET_normalized*600, alpha=0.045, label='50%',color='blue')
            ax.scatter(wind_direction_rad, distance_30, s=ET_normalized*600, alpha=0.045, label='30%',color='orange')
            ...
            ax.legend()
            plt.title('ET Distribution map')
            st.pyplot(fig)
            plt.close()
            st.write("Fetch distance (m): Scale shown on South-West line; Larger circle == Higher ET;")
